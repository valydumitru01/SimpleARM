...
typedef enum InstructionType {
    /// ## Data Processing (AND/EOR/ORR/BIC/MOV/MVN/TST/TEQ/CMP/CMN/etc.) ##
    ///
    ...
    /// ## Multiply Long / Multiply-Accumulate Long (UMULL/SMULL, UMLAL/SMLAL) ##
    ///
    /// Encoding (A32, 32 bits):
    /// - [31:28]: Cond
    /// - [27:25]: 000
    ...
    ///
    /// 2) 01 (bits 27:26)
    ///    Class selector for single data transfer instructions.
    ///
    /// 3) I (bit 25) - Immediate/Register offset selector
    ///    0 = immediate offset (12-bit unsigned immediate in Offset[11:0])
    ///    1 = register offset (Rm with optional shift encoded in Offset[11:0])
    ///
    /// 4) P (bit 24) - Pre/Post indexing
    ///    1 = pre-index (address formed before transfer)
    ///    0 = post-index (address uses base first, then base updated)
    ///
    /// 5) U (bit 23) - Up/Down
    ///    1 = add offset to base
    ///    0 = subtract offset from base
    ///
    /// 6) B (bit 22) - Byte/Word transfer
    ///    0 = word (32-bit) transfer
    ///    1 = byte (8-bit) transfer (zero-extended on load)
    ///
    /// 7) W (bit 21) - Write-back
    ///    1 = write updated address back into Rn
    ///    0 = do not write back
    ///    Note: for post-indexed form (P=0), write-back is implied; in that case W is
    ///    typically 0 in assembly syntax, but the core behavior is still “base updated”.
    ///
    /// 8) L (bit 20) - Load/Store
    ///    1 = LDR (load from memory into Rd)
    ///    0 = STR (store Rd to memory)
    ///
    /// 9) Rn (bits 19:16)
    ///    Base register.
    ///
    /// 10) Rd (bits 15:12)
    ///     Transfer register (destination for loads, source for stores).
    ///
    /// 11) Offset (bits 11:0)
    ///     If I=0 (immediate offset):
    ///       - imm12 in bits [11:0] (unsigned byte offset)
    ///     If I=1 (register offset / scaled register):
    ///       - Rm in bits [3:0]
    ///       - Shift type in bits [6:5]
    ///       - Shift amount is either:
    ///           * immediate (bits [11:7]) when bit[4]=0, or
    ...
    ///
    /// Addressing semantics (conceptual):
    ///   offset = (I==0) ? imm12 : ShifterOperand(Rm, shift_spec)
    ///   delta  = (U==1) ? +offset : -offset
    ///
    ///   if (P==1) {
    ///     addr = Rn + delta;
    ...
    ///   } else {
    ///     addr = Rn;
    ///     Rn   = Rn + delta;   // post-index updates base after transfer
    ///   }
    ///
    /// Transfer semantics:
    ///   if (L==1) {                    // LDR
    ///     if (B==0) Rd = Mem32[addr];  // word
    ///     else      Rd = ZeroExtend8(Mem8[addr]);   // byte
    ///   } else {                       // STR
    ///     if (B==0) Mem32[addr] = Rd;
    ///     else      Mem8[addr]  = Rd[7:0];
    ///   }
    ///
    /// Notes / restrictions (ARM7TDMI / ARMv4-class):
    ///   - If Rn == R15, the base value used is PC + 8 (ARM-state prefetch bias).
    ///   - Using R15 as Rd has special behavior:
    ///       * LDR to R15 is a control-flow change (pipeline flush); alignment/state rules apply.
    ///       * STR from R15 stores PC + 12 on ARM7TDMI (value is implementation-defined on some cores).
    ///   - Word transfers ideally use word-aligned addresses; misalignment behavior is core-specific
    ///     (ARM7TDMI rotates on unaligned LDR word; STR word alignment is also implementation-defined).
    ///   - Register-offset form uses the same barrel shifter rules as data processing (carry-out rules
    ///     matter for some cores if you model internal address calc flags, though CPSR is not updated).
